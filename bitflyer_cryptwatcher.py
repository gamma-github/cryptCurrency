...
while True:
    ...
    if datetime.now().strftime('%S') [0:2]== '00':
        # データの更新
        tick = bfapi.ticker(product_code="BTC_JPY")
        board = bfapi.board(product_code="BTC_JPY")
        print("{}".format(json.dumps(tick,indent=4)))
        raws = np.append(raws, tick['ltp'])
        timestamp = np.append(timestamp, tick['timestamp'])
        mid_price = np.append(mid_price, board['mid_price'])
        # 次の00秒まで休憩
        time.sleep(57)
        ...
        if elapsed_time > 60*t:#t分後にデータを処理してbreak
            ...
            print(timestamp)
            print(mid_price)
            result = zip(timestamp,mid_price,raws)
            f = open('result_bitflyer.txt', 'w')
            for x in result:
                f.write(str(x) + "\n")
            f.close()
            #pandasを使って処理
            df = pd.DataFrame({'mid_price':mid_price,'raws':raws}, index = timestamp)
            print(df)
            #csv形式で保存
            df.to_csv("result_bitflyer.csv")
            break
